---
title: Creating beautiful screenshots of your code with a serverless function
subtitle: Programmatic Carbon images generation from a simple API
date: '2020-06-02T08:00:00.000Z'
categories: []
keywords: ['vercel', 'serverless', 'function', 'headless', 'chrome', 'screenshot', 'carbon', 'code', 'snippet']
slug: creating-beautiful-screenshots-source-code-with-serverless-function
featured: true
colorFeatured: '#2B2D3E'
---
...
I've used carbon.now.sh quite a bit in the past, and what I've noticed was that the code snippet and the settings I set on the website are automatically added as query parameters to the URL.
...
Once the data would be received by the function, it would need to "translate" it to a carbon valid URL. I wrote the following function "getCarbonUrl" to take care of that:
...
- making the "code string" URL safe using `encodeURIComponent` which will encode any special characters of the string
- detecting the language: for this I could either look for any `language` query param, detect it based on the code string or fall back to `auto` which and let carbon figure out the language.
- taking the rest of the query string and append them to the URL

I now had a valid Carbon URL 🎉. Now to automate the rest, I would need to paste the URL in a browser which would give the corresponding image of it and take a screenshot.

## Running a headless Chrome in a serverless function

This step is the core and most interesting part of this implementation. I was honestly pretty mind blown to learn it was possible to run a headless chrome browser in a serverless function, to begin with.
For this, I used [chrome-aws-lambda](https://github.com/alixaxel/chrome-aws-lambda) which despite its name or what's specified in the project's README, seems to work really well on any serverless provider (in the next part you'll see that I used Vercel to deploy my function, and I was able to get this package running on it without any problem).
...
3. Set the viewport, here I set it to something super big to make sure that the target is contained within the browser "window".
4. Navigate to the URL we generated in the previous step
5. Look for an HTML element with the id `export-container`, this is the div that contains our image. (SCREENSHOT)
...
Now that the function was built, it was deployment time 🚀! This time I chose to give [Vercel](https://vercel.com/) a try to test and deploy this serverless function on their service. However, there was a couple of things I needed to do first:

- Put all my code in an `api` folder
- Have the main request handler in an index file
...
<!---
or here's a small playground where you can call it and get back a screenshot for any code snippet you'd type in the input.
```js live
  const url = 'https://carbonara-nu.now.sh/api/carbonara';
  ...
  async function doFetch(data) {
    ...
    while (true) {
      ...
      console.log(value);
      let base64String = String.fromCharCode(...new Uint8Array(value)).replace(
        /\"/g,
        ''
      );
      return base64String;
  ...
  const Form = () => {
    ...
    const [valueToSubmit, setValueToSubmit] = React.useState('');
    const [imageData, setImageData] = React.useState(null);

    React.useEffect(() => {
      if (valueToSubmit !== '') {
        doFetch(valueToSubmit).then(foo => {
          setImageData(foo);
        });
      }
    }, [valueToSubmit]);
    ...
    return (
      <>
        ...
        <Button primary onClick={() => setValueToSubmit(btoa(text))}>
          Submit{' '}
        </Button>
        <Button secondary onClick={() => setImageData('')}>
          Clear{' '}
        </Button>
        <div>
          {imageData ? (
